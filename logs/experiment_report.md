...
## 2026-03-27 / Exp-15 / 中期报告四页排版产出（论文/PPT）

### 实验目的
将既有实验日志与指标表整理为可直接复用的中期汇报材料，形成“摘要-方法-结果-结论”标准四页结构与精简展示版本。

### 实验方法
- 从统一日志与评估文件中提取同口径指标（class-aware，100 图）。
- 生成两份报告文档：
  - 完整版四页稿（适合论文正文与详细讲解）；
  - 精简版四页稿（每页 3-5 条，适合正式答辩展示）。
- 校验关键数字与现有 official 评估一致，避免口径漂移。

### 实验结果
- 产出文件：
  - `project_records/summaries/midterm_report_ppt_4pages_20260327.md`
  - `project_records/summaries/midterm_report_ppt_4pages_concise_20260327.md`
- 汇报中使用的关键指标（沿用当前有效结果）：
  - 默认整图基线：Acc@0.5=0.3077，Acc@0.75=0.2444，mAP@0.5=0.1367
  - best 整图：Acc@0.5=0.3443，Acc@0.75=0.2624，mAP@0.5=0.1451
  - 滑窗 bestcfg：Acc@0.5=0.3760，Acc@0.75=0.2813，mAP@0.5=0.1517
  - Exp-14（当前综合最优）：Acc@0.5=0.4113，Acc@0.75=0.3021，mAP@0.5=0.1685
- 结论：已完成可直接用于论文与答辩展示的标准化中期材料封装，本次为文档归档与结果结构化，不新增模型训练与推理实验。
